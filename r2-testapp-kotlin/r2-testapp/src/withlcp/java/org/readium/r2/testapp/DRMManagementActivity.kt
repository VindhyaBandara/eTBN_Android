...
//                    }
                    ...
                    //if ((drmModel.start != null && drmModel.end != null) && drmModel.start != drmModel.end) {by vindhya
                    if ((drmModel.issued != null && drmModel.end != null) && drmModel.issued != drmModel.end) {
                        linearLayout {
                            orientation = LinearLayout.HORIZONTAL
                            lparams(width = matchParent, height = wrapContent)
                            weightSum = 2f
                            ...
                            textView {
                                padding = dip(10)
                                //text = drmModel.start?.toString(DateTimeFormat.shortDateTime())vindhya
                                text = drmModel.issued?.toString(DateTimeFormat.shortDateTime())
                                textSize = 18f
                                gravity = Gravity.END
                            }.lparams(width = wrapContent, height = wrapContent, weight = 1f)
...
//                            typeface = Typeface.DEFAULT_BOLD
//                        }
                        /*************************Temporaly commented to hide RENEW and RETURN***************************/
