...
interface LcpService {
     ...
     */
    suspend fun acquirePublication(lcpl: ByteArray,usertoken:String): Try<AcquiredPublication, LcpException>

    /**
     *  Acquires a protected publication from a standalone LCPL file.
     */
    suspend fun acquirePublication(lcpl: File,usertoken:String): Try<AcquiredPublication, LcpException> = withContext(Dispatchers.IO) {
        try {
            acquirePublication(lcpl.readBytes(),usertoken)
        } catch (e: Exception) {
    ...
    fun importPublication(lcpl: ByteArray, authentication: LcpAuthenticating?, completion: (AcquiredPublication?, LcpException?) -> Unit) {
        GlobalScope.launch {
            acquirePublication(lcpl,"usertoken")
