...
internal class LicensesService(
    ...
    override suspend fun acquirePublication(lcpl: ByteArray,usertoken:String): Try<LcpService.AcquiredPublication, LcpException> =
        try {
            val licenseDocument = LicenseDocument(lcpl)
            if (DEBUG) Timber.d("license ${licenseDocument.json}")
            fetchPublication(licenseDocument,usertoken).let { Try.success(it) }
        } catch (e: Exception) {
    ...
    private suspend fun fetchPublication(license: LicenseDocument,usertoken:String): LcpService.AcquiredPublication {
        ...
        val format = network.download(url, destination,usertoken) ?: MediaType.of(mediaType = link.type) ?: MediaType.EPUB
