    ...
    companion object {
        ...
        fun parseURL(url: URL): Promise<ParseData, Exception> {
            return Fuel.get(url.toString(), null).promise() then {
                val (_, _, result) = it
                this.parse(xmlData = result, url = url,reccount=0)
            }
        }
        ...
        @Suppress("unusedpost")
        fun parseURL(headers: MutableMap<String,String>, url: URL,reqparams:String): Promise<ParseData, Exception> {
            val paramlist:List<Pair<String,Any?>>
            val templist :List<String>
            templist=reqparams.split(",")

                val hMap: MutableMap<String, Any?> = HashMap()
                hMap["page"] = templist[0]
                hMap["size"] = templist[1]
                //hMap["keyword"] = templist[2]
                hMap["orderColumn"] = templist[2]
                hMap["isASC"] = templist[3]
                hMap["skip"] = templist[4]
                hMap["keyword"] = templist[5]

                paramlist=hMap.toList();

            return Fuel.post(url.toString(), paramlist).header(headers).promise() then {
//                val (_, _, result) = it
                val (_, _, result) = it
                        var totcount = it.second.headers["record-count"]
                var numofitems : Int = totcount.elementAt(0).toInt()
                //record-count
                this.parse(xmlData = result, url = url,reccount = numofitems)
                //this.parse(xmlData = result, url = url,reccount = 10)
            }
        }

        fun parse(xmlData: ByteArray, url: URL,reccount:Int): ParseData {//*************************************reccount is added by vindhya
            val root = XmlParser().parse(xmlData.inputStream())
            return if (root.name == "feed")
                ParseData(parseFeed(root, url,reccount), null, 1)
            else
                ParseData(null, parseEntry(root, url), 1)
        }

        private fun parseFeed(root: ElementNode, url: URL,reccount:Int): Feed {
            ...
            /*************************************added by vindhya**********************************************/
            feed.metadata.numberOfItems=reccount
            /*************************************added by vindhya**********************************************/
